...
RUN apt-get update \
    ...
    && apt-get update \
    && apt-get install -y google-chrome-stable=103.0.5060.53-1 fonts-ipafont-gothic fonts-wqy-zenhei fonts-thai-tlwg fonts-kacst fonts-freefont-ttf libxss1 --no-install-recommends \
...
RUN npm install puppeteer@15.3.0 \
